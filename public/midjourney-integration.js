...
/**
 * Configuration object for Midjourney integration
 * @type {Object}
 * @property {string} webhookUrl - Discord webhook URL where Midjourney bot is present
 * @property {string} commandPrefix - Command prefix for Midjourney
 * @property {number} maxPromptLength - Maximum prompt length to prevent abuse
 * @property {number} minRequestInterval - Minimum time between requests in milliseconds
 */
const config = {
 ...
 * @returns {boolean} True if the URL is a valid Discord webhook URL
 */
...
/**
 * Sanitize user input for prompts to prevent injection attacks and abuse
...
/**
 * Generate an image using Midjourney via Discord webhook
 * @async
 * @param {string} prompt - The prompt to send to Midjourney
 * @returns {Promise<Object>} The response data from Discord
...
async function generateMidjourneyImage(prompt) {
  ...
  if (now - lastRequestTime < config.minRequestInterval) {
    const waitTime = Math.ceil(
      (config.minRequestInterval - (now - lastRequestTime)) / 1000
    );
  ...
  if (!config.webhookUrl || config.webhookUrl === 'YOUR_DISCORD_WEBHOOK_URL') {
    const errorMsg =
      'Discord webhook URL not configured. Please set it in the midjourney-integration.js file.';
  ...
  if (!isValidWebhookUrl(config.webhookUrl)) {
    const errorMsg =
      'Invalid Discord webhook URL format. Please check your configuration.';
    console.error(errorMsg);
    showToast(errorMsg, 5000, 'error');
    return Promise.reject(new Error(errorMsg));
  }
  ...
  try {
    ...
    if (typeof axios !== 'function' && (!axios || !axios.post)) {
      throw new Error(
        'Axios library not loaded. Make sure axios is included before this script.'
      );
    }
    ...
    showToast(
      'Prompt sent to Midjourney! Check your Discord channel for the generated image.',
      3000,
      'success'
    );
    ...
    if (error.code === 'ECONNABORTED') {
      ...
      if (status === 400) {
        ...
        errorMessage +=
          'Webhook not found. Please verify your Discord webhook URL.';
      } else if (status === 429) {
        errorMessage +=
          'Rate limited by Discord. Please wait before trying again.';
        errorType = 'warning';
      } else if (status >= 500) {
      ...
      errorMessage +=
        'No response from Discord server. Please check your internet connection and webhook URL.';
    } else {
...
/**
 * Generate an image based on the current beer description
 * Uses the globally available currentBeer object set by main.js
 * @returns {void}
 */
function generateImageForCurrentBeer() {
  try {
    ...
    if (typeof showToast !== 'function') {
      console.error(
        'showToast function not found. Make sure main.js is loaded before this script.'
      );
      alert('Error: Toast notification system not available');
      return;
    }

    // Check if currentBeer exists in the global state set by main.js
    const beer = window.currentBeer;
    if (!beer) {
    ...
    if (!beer.name) {
      showToast(
        'Beer name not found in the current beer object',
        3000,
        'error'
      );
      return;
    ...
    generateMidjourneyImage(prompt)
      ...
      .finally(() => {
        ...
        if (btn) {
          ...
          btn.innerHTML =
            '<i class="fas fa-image" aria-hidden="true"></i> Generate Image';
        }
      });
  } catch (error) {
...
/**
 * Set up event listeners for image generation button
 * @returns {void}
 */
...
/**
 * Initialize the Midjourney integration functionality
 * Sets up event listeners when the DOM is ready
 * @returns {void}
 */
